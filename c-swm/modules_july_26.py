...
class ContrastiveSWM(nn.Module):
    ...
    def __init__(self, embedding_dim, input_dims, hidden_dim, action_dim,
        ...
        self.inverse_model = Inverse(
            input_dim=num_objects*embedding_dim*2,
            hidden_dim=hidden_dim // 2,   # 256 to test 512??
            action_dim=1)   # action dim 4(one hot ) or 1  ??
    ...
    def inverse_loss(self, states, actions, next_states):
        states_cat = torch.cat((states, next_states), 2)
        pred_actions = self.inverse_model(states_cat)

        action = actions.unsqueeze(1)
        action = action.type(torch.cuda.FloatTensor)

        loss = F.mse_loss(pred_actions, action) / states.size(0)

        # diff = pred_action - action  # original action or one-hot action??

        return loss  #  *10   ## dicount factor??

    def fill_act(self, actions, index):
        actions_copy = actions.clone()
        fill_number = actions_copy[index].item()
        actions_copy = actions_copy.fill_(fill_number)
        return actions_copy

    def inverse_loss_contr(self, states, actions, next_states):
        states_cat = torch.cat((states, next_states), 2)
        pred_actions = self.inverse_model(states_cat)

        loss_cont = []
        for i in range(pred_actions.size(0)):
            act = self.fill_act(pred_actions, i)
    ...
    def contrastive_loss(self, obs, action, next_obs):
        ...
        # self.inv_loss = self.inverse_loss(state, action, next_state)
        self.inv_loss = self.inverse_loss_contr(state, action, next_state)

        loss = self.pos_loss + self.neg_loss + self.inv_loss
    ...
    Define a 2-layer GNN model.
     ...
     def __init__(self, in_feats, hidden_size, num_classes):
        super(GNN, self).__init__()
            self.gcn1 = GNNLayer(in_feats, hidden_size)
            self.gcn2 = GNNLayer(hidden_size, num_classes)
    def forward(self, obs):
        h = self.act1(self.ln1(self.cnn1(obs)))
