...
class ContrastiveSWM(nn.Module):
    ...
    def inverse_loss(self, obs, action, next_obs):
        # pred_action = self.inverse_model(obs, next_obs)
        # diff = pred_action - action  # original action or one-hot action??

        return 0

    def inverse_loss_state(self, state, action, next_state):
        return 0
    ...
    def contrastive_loss(self, obs, action, next_obs):
        ...
        self.inv_loss = self.inverse_loss(obs, action, next_obs)
